...
for( mes.ref in meses){
  ...
  for(nclus in 1:length(cluster.name)){
    ...
    Formulas <- sapply(id,function(i)
      ...
      paste("y.dato ~ ",paste0("s(",predictor.name[i],",k=3)",collapse="+"))
    )
    rm(fit.final)
    umbral=1000000
    resultado.modelo=data.frame()
    coef=list()
    ...
    for(fml in 1:length(Formulas)){
      ...
      fit = gam(as.formula(Formulas[fml]), method = "REML",data=predictores)
      errors = deviance(fit)
      R2=round(Rsquared(fit$fitted.values,y.dato),3)
      Rbar2=round(RAdj(R2,length(y.dato),length(fit$coefficients)-1),3)
      #resultado.modelo <- rbind(resultado.modelo,R2,t(errors))
      resultado.modelo <- rbind(resultado.modelo,data.frame(R2,Rbar2))
    ...
    colnames(resultado.modelo2)[5] ="Coeficients"
    ...
    write.xlsx(resultado.modelo2,file=paste0("Modelos.",mes.ref,"_",miny,"_",maxy,"/Cluster",nclus,"/Modelos_",mes.ref,"_C",nclus,"_LASSO_GAM.xlsx"))
